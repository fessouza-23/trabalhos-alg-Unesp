...
void printArrayData(SortFunction bufferAlgos[], int vet[], int n, const char* sortName[]) {
    int comps, trocas, arrayCompsAndTrocas[MAX_ARGS];
    ...
    for (i = 0; i < SORT_FUNCTIONS; i++) {
        comps = 0; trocas = 0;
        printf("--- %s ---\n", sortName[i]);
        getSortData(bufferAlgos[i], vet, 0, n-1, &comps, &trocas, arrayCompsAndTrocas);
    }
...
void getSortData(int* (*f)(int*, int, int, int*, int*, int*), 
...
int* bubble_sort(int vet[], int m, int n, int *comps, int *trocas, int *arrayCT) {
...
int* insertion_sort(int vet[], int m, int n, int *comps, int *trocas, int *arrayCT) {
    int i, j, aux;
...
int* selection_sort(int vet[], int m, int n, int *comps, int *trocas, int *arrayCT) {
    int i, j, min_ind;
...
int* shell_sort(int vet[], int m, int n, int *comps, int *trocas, int *arrayCT) {
    int i, j, k, aux;
...
int* quick_sort(int v[], int left, int right, int *comps, int *trocas, int *arrayCT) {
	if (left < right) {
        ...
        int index_pivot = partition(v, left, right, comps, trocas, arrayCT)[2];
...
int* bead_sort(int vet[], int m, int n, int *comps, int *trocas, int *arrayCT) {
    int i, j, max, soma;
    ...
    for (i = len - soma; i < len; i++) {
        //printf("TESTE BEAD 11\n");
        CONTA(i, j) = 1;
        //printf("TESTE BEAD 12\n");
        (*trocas)++;
    }
